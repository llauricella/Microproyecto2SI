...
export default function RutasSuscritas() {
    ...
    useEffect(() => {
        const fetchRutas = async () => {
            try {
                ...
                const rutasSuscritas = userDoc.data().rutas || [];
                setRutas(rutasSuscritas);
            } catch (error) {
                console.error("Error al obtener las rutas suscritas:", error.message);
                setError("Ocurrió un error al obtener las rutas suscritas.");
            }
        };
    ...
    return (
        <div className="flex flex-col items-center min-h-screen p-4 md:p-8">
            <h1 className="text-3xl md:text-4xl font-bold mb-8 bg-gray-100 rounded-xl p-2 text-center">Rutas Suscritas</h1>
                ...
                {filtrarRutas().map((ruta, index) => (
                    <div key={index} className="bg-white p-6 rounded-lg shadow-lg">
                        ...
                        <p className="text-lg mb-2">
                            <span className="font-semibold">Fecha:</span> {new Date(ruta.fecha).toLocaleDateString()}
                        </p>
                        <p className="text-lg mb-2"><span className="font-semibold">Guía:</span> {ruta.guia}</p>
                        <p className="text-lg mb-2"><span className="font-semibold">Precio:</span> ${ruta.precio}</p>
                    </div>
                ))}
            </div>
        </div>
